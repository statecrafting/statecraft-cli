...
extends:
  # 000 establishes the governance workflow; 009 replaces its body with the
  # kit's govern.yml chain under the filename 000 chose, so 000 remains the
  # owner of "there is a governance gate on every PR" and 009 owns its shape.
  - { spec: "000-bootstrap", unit: ".github/workflows/spec-spine.yml", nature: additive }
  # 000 owns the configuration file; 009 adds the version pin, the hashed
...
- `.github/workflows/spec-spine.yml` (extends spec 000): the kit
  `govern.yml` body under the filename 000 established.
- `spec-spine.toml` (extends spec 000): `[meta] required_version`, the
...
`--fail-on-unresolved` is off by decision (spec-spine spec 050): spec 008 is
...
-D warnings`, `cargo test`, release build) stays in `ci.yml` (spec 002);
...
16/16 source files specifically claimed once spec 002 claims `tests/`
