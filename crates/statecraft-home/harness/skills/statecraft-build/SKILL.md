---
name: statecraft-build
description: "Implement one spec start to finish per AGENTS.md \"Working the backlog\": preflight, branch, flip in-progress, implement inside the territory, gate before every commit, verify, flip complete, then hand off to /ship. Applies only inside a Statecraft project: a repository holding `.statecraft/environment.json`. Outside one, ignore this file entirely."
