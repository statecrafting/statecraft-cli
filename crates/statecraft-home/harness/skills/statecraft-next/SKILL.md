---
name: statecraft-next
description: "Name the next spec to build: the ready set from spec-spine registry plan, minus anything a human has not approved, with in-flight specs and honest blockers reported separately. Read-only. Applies only inside a Statecraft project: a repository holding `.statecraft/environment.json`. Outside one, ignore this file entirely."
