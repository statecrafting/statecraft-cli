---
name: statecraft-code-review
description: "Review the current diff for correctness bugs, spec drift, and illegitimate mid-build spec edits, with the governed gate as evidence, and emit an evidence-oriented findings list. Applies only inside a Statecraft project: a repository holding `.statecraft/environment.json`. Outside one, ignore this file entirely."
