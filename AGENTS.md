...
This repository holds eight crates and one binary, each crate claimed by the
...
moved there from `.derived/` with spec `010`; the whole of `.statecraft/` is
committed except `.statecraft/state/`.
...
| `.statecraft/derived/**` | **compiler** | Regenerated by `make refresh` only. Committed. Never hand-edited. `build-meta.json` is the one gitignored file. Moved from `.derived/` by spec `010`. |
